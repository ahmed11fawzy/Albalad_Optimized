...
const complementaryColors = [
  '#b88c36', '#d4a437', '#8b6f2a', '#e0b565', '#a67c00',
  '#c89b3c', '#f4c430', '#8c5523', '#d9a441', '#b88c36',
];

const getRandomColor = () => {
  return complementaryColors[Math.floor(Math.random() * complementaryColors.length)];
};
...
const CustomDropDownMenu = () => {
  ...
  const { data, isLoading: isLoadingCategories, isFetching, error } = useGetCategoriesQuery(currentPage);
  ...
  const handleMenuScroll = useCallback((event) => {
    ...
    scrollTimeoutRef.current = setTimeout(() => {
      ...
      if (scrollPercentage > 50 && hasMoreCategories && !isLoadingMore && !isFetching) {
  ...
  }, [loadMoreCategories, hasMoreCategories, isFetching, isLoadingMore, anchorEl]);
        ...
        >
          ...
          >
            {isLoadingCategories && !categories.length ? (
              ...
              <>
                <CategoriesGrid>
                  {categories.map((category) => ( category.children_all && category.children_all.length > 0 ) && (
                    <CategoryBox key={category.id} className="category-box">
                      ...
                      >
                        <CategoryHeader>
                          {category.image ? (
                            ...
                            <CategoryIcon sx={{ fontSize: 24, color: getRandomColor() }} />
                          )}
                          <Typography
                            variant="subtitle1"
                            sx={{ fontWeight: 600, color: getRandomColor() }}
                          >
                            {category.name}
                          </Typography>
                        </CategoryHeader>
                      ...
                      <SubCategoriesWrapper isOpen={expandedCategories[category.id]}>
                        {category.children_all && category.children_all.length > 0 ? (
                          ...
                          <StyledMenuItem disabled>لا توجد فئات فرعية</StyledMenuItem>
                        )}
                      </SubCategoriesWrapper>
                    </CategoryBox>
                  ))}
