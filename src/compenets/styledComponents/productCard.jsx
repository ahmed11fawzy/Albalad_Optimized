...
import { Card, CardContent, CardMedia, Typography, Box, Chip, IconButton, Badge } from '@mui/material';
...
const StockBadge = styled(Chip)(({ stocklevel }) => ({
  fontSize: '10px',
  height: 20,
  backgroundColor: stocklevel === 'low' ? ' #ff9800' : 
                   stocklevel === 'out' ? ' #f44336' : 'rgb(204, 156, 61)',
  color: 'white',
  fontWeight: 'bold',
  borderRadius: '10px',
}));
...
const ProductCard = ({ product }) => {
  ...
  const discountPercentage = product.old_price > product.price 
  ...
  const stockLevel = getStockLevel(product.quantity, product.low_stock_quantity);
  const stockText = {
    'out': 'نفدت الكمية',
    'low': 'كمية محدودة',
    'good': 'متوفر'
  ...
  return (
    ...
    <StyledCard >
      ...
      <CardContent sx={{  padding: '16px' }}>
        ...
        >
          {product.description?.replace(/<[^>]*>/g, '') || 'منتج عالي الجودة'}
        </Typography>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' }}>
          ...
          <PriceContainer className="price-container">
            <Box sx={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <PrimaryPrice>
                {parseInt(product.price)} ر.س
              </PrimaryPrice>
              {discountPercentage > 0 && (
                <OldPrice>
                  {parseInt(product.old_price)} ر.س
                </OldPrice>
              )}
            </Box>
          </PriceContainer>
